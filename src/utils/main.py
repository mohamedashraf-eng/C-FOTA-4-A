...
class prj_foem_sqlite:
    ...
    def __get_row_in_table(self, table_name: str, row_id: int):
        ...
        logging.info(
            f"Fetched row from table {table_name} column, row_id={row_id}")
        self.__close_db()
        return row
...
class prj_foem_sqlite_firmware(prj_foem_sqlite):
    ...
    def __frimware_calculate_hash(self):
        ...
        if hash_type == "SHA2-256":
            ...
            logging.info(
                f"created a hash [CMAC] for firmware {self.__firmware_hash}")
        elif hash_type == "HMAC":
            self.__firmware_hash = self.__calculate_hmac()
            logging.info(
                f"created a hash [HMAC] for firmware {self.__firmware_hash}")
        else:
            logging.critical(
    ...
    def row(self, row):
        ...
        self._prj_foem_sqlite__insert_new_row_in_table(
            self.__table_name, **row)
...
def main():
    ...
    # MQTT
    main_mqtt = prj_foem_mqtt(
        yaml_mqtt_cfg=main_yaml.get_mqttcfg, manual_mqtt_cfg=None)
    # main_mqtt.run()
    # CMD_ANALYZER
