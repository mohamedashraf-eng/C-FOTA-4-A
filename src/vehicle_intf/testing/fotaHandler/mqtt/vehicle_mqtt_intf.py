...
class SimpleMQTTClient:
    ...
    def __sendNack(self, ErrorCode):
        self.__sendToOEM(f"{SimpleMQTTClient.FotaVehicleResponse.ACK.value}, {ErrorCode}")
    ...
    def __fota_cmd_handle_UPDATE_REQUEST(self):
        ...
        while not self.__mqtt_currmsg == SimpleMQTTClient.FotaCommands.SEND_FIRMWARE_DONE.value:
            self.__firmwareRx.append(self.__mqtt_currmsg) 
        
        self.__sendAck()
        
        time.sleep(5)
        if self.__mqtt_currmsg == SimpleMQTTClient.FotaCommands.SEND_FIRMWARE_DONE.value:
            # print(self.__firmwareRx)
            # Send HASH
            self.__sendToOEM(SimpleMQTTClient.FotaVehicleResponse.SEND_FIRMWARE_HASH.value)
            while not self.__mqtt_currmsg == SimpleMQTTClient.FotaCommands.SEND_FIRMWARE_HASH_DONE.value:
                pass
    ...
    def __save_firmware(self):
        pass
        
    def __vehicle_cmd_handle(self):
        self.__fota_cmd_exec(self.__mqtt_currmsg)
        # if self.__mqtt_currmsg in self.__expected_oem_cmds:
        #     self.__fota_cmd_exec(self.__mqtt_currmsg)
        # else:
        #     self.__sendNack(ErrorCode=SimpleMQTTClient.FotaErrorCode.INVALID_CMD.value)
        #     print(f"Invalid command sent: {self.__mqtt_currmsg}")
    ...
    def run(self):
        ...
        try:
            while True:
                try:
                    if self.__mqtt_currmsg in self.__expectedCmds:
